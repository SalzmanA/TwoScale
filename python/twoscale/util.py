...
if _ts_dolfinx_exist:
    ...
    def createFineScaleSytems(
        ...
        forma = fem.forms.form(
            a, dtype=PETSc.ScalarType
        )  # compiler_option and jit not provided for now TODO
        formad = fem.forms.form(
            a, dtype=PETSc.ScalarType
        )  # compiler_option and jit not provided for now TODO
        formb = fem.forms.form(
            b, dtype=PETSc.ScalarType
        )  # compiler_option and jit not provided for now TODO
        formbd = fem.forms.form(
            b, dtype=PETSc.ScalarType
        )  # compiler_option and jit not provided for now TODO
        ...
        if MPC is not None:
            ...
            pattern.finalize()
            A = dolfinx_cpp.la.petsc.create_matrix(
                MPC.function_space.mesh.comm, pattern
            )
            AD = dolfinx_cpp.la.petsc.create_matrix(
                MPC.function_space.mesh.comm, pattern
            )
            # generate rhs
            B = la.petsc.create_vector(
                [
                    (
    ...
    def root_mesh(dom: mesh.Mesh):
        ...
        if len(topox) > 0:
            ...
            global_dofsx = dom.geometry.index_map().local_to_global(
                topox[topology_dofsx].copy()
            )
            topox[topology_dofsx] = global_dofsx
            global_topologyx = dom.comm.gather(
                topox[: (num_dofsx_per_cell + 1) * num_cellsx_local], root=0
            )
            global_geometryx = dom.comm.gather(
                geomx[: dom.geometry.index_map().size_local, :], root=0
            )
            global_ctx = dom.comm.gather(cellx[:num_cellsx_local])
        else:
            global_topologyx = dom.comm.gather(np.empty(0, dtype=int), root=0)
            global_geometryx = dom.comm.gather(
                np.empty((0, 3), dtype=dom.geometry.x.dtype), root=0
            )
    ...
    def root_rank(dom: mesh.Mesh):
        ...
        spaceG = fem.functionspace(
            dom,
            basix.ufl.element(
                "DG", dom.ufl_domain().ufl_coordinate_element().cell_type, 1
            ),
        )
        ...
        num_dofsx_local = (
            spaceG.dofmap.index_map.size_local * spaceG.dofmap.index_map_bs
        )
        if len(topox) > 0:
            num_dofsx_per_cell = topox[0]
            topology_dofsx = (np.arange(len(topox)) % (num_dofsx_per_cell + 1)) != 0
            global_dofsx = spaceG.dofmap.index_map.local_to_global(
                topox[topology_dofsx].copy()
            )
            topox[topology_dofsx] = global_dofsx
            global_topologyx = dom.comm.gather(
            ...
            global_geometryx = dom.comm.gather(
                geomx[: spaceG.dofmap.index_map.size_local, :], root=0
            )
            global_ctx = dom.comm.gather(cellx[:num_cellsx_local])
    ...
    # =================================================================
    def root_field(field: fem.Function, scale, extrude=0):
        """
        ...
        if len(topox) > 0:
            num_dofsx_per_cell = topox[0]
            topology_dofsx = (np.arange(len(topox)) % (num_dofsx_per_cell + 1)) != 0
            global_dofsx = space.dofmap.index_map.local_to_global(
                topox[topology_dofsx].copy()
            )
            topox[topology_dofsx] = global_dofsx
            global_topologyx = dom.comm.gather(
            ...
            global_geometryx = dom.comm.gather(
                geomx[: space.dofmap.index_map.size_local, :], root=0
            )
            ...
            global_valsx = dom.comm.gather(
                np.empty(0, dtype=field.x.array.dtype), root=0
            )
        if dom.comm.rank == 0:
            root_geom = np.vstack(global_geometryx)
            root_top = np.concatenate(global_topologyx)
            ...
            if extrude > 0 and dom.topology.dim == 2:
                ...
                if extrude > 1:
                    if nbv > 1:
                        valuesx[:, dom.topology.dim] = np.heaviside(
                            np.square(tmp[:, 0]) + np.square(tmp[:, 1]), 0
                        )
                    else:
                        valuesx[:, dom.topology.dim] = np.heaviside(
                            np.square(tmp[:, 0]), 0
                    ...
                    if nbv > 1:
                        valuesx[:, dom.topology.dim] = np.sqrt(
    ...
    # =================================================================
    def par_field(field: fem.Function, scale, extrude=0):
        """
    ...
    # =================================================================
    def all_sub(space: fem.FunctionSpace, dom: mesh.Mesh, subs, psid, L=0, k=0):
        if space.mesh == dom:
    ...
    def show_dofs_ids(
        ...
        * related to coarse scale with enrichment
        ...
        loc_label = np.array(
            [
                i * bs + b
                for i in range(idxmaps.size_local + idxmaps.num_ghosts)
                for b in range(bs)
            ]
        )
        glob_label_loc = np.array([i for i in range(idxmaps.size_local)])
        glob_label_loc = idxmaps.local_to_global(glob_label_loc)
        glob_label = np.concatenate(
            (
                np.array(
                    [i * bs + b for i in glob_label_loc for b in range(bs)],
                    dtype=np.int64,
                ),
                np.array(
                    [i * bs + b for i in idxmaps.ghosts for b in range(bs)],
                    dtype=np.int64,
                ),
            )
        )
    ...
    def show_patch_sol(
        ...
        for seq in list_seq:
            ...
            if raw:
                ...
                plt.add_title(
                    "seq {} (proc{})".format(seq, domain.comm.rank), font_size=7
                )
                plt.add_axes()
                if domain.comm.rank == 0:
                    plt.subplot(1, 0)
                ...
                space_patch = fem.functionspace(
                    sdom[0], sdom[0].ufl_domain().ufl_coordinate_element()
                )
                field_patch = fem.Function(space_patch)
                cell_over = np.arange(
                    sdom[0].topology.index_map(dim).size_local, dtype=int
                )
                ...
                if save_to_file:
                    spaceG = fem.functionspace(
                        ...
                        basix.ufl.element(
                            "DG",
                            sdom[0].ufl_domain().ufl_coordinate_element().cell_type,
                            1,
                        ),
                    )
                    procfield = fem.Function(spaceG, name="proc")
                    ...
                    with VTXWriter(
                        field_patch.function_space.mesh.comm,
                        f"{to_file}_{seq}_proc.bp",
                        [procfield],
                    ) as ofile:
                        ofile.write(0.0)
                    with VTXWriter(
    ...
    # =================================================================
    def rootMasterFace(sj: core.scaleJump, dom: mesh.Mesh):
        """
        ...
        sub = mesh.create_submesh(
            dom, dimm1, mf[mf < dom.topology.index_map(dimm1).size_local]
        )
    ...
    # =================================================================
    def distanceFromSphere(p: npt.ArrayLike, Orig: npt.ArrayLike, r: float):
        """
    ...
    # =================================================================
    def distanceFromParallelogram(
        p: npt.ArrayLike, Orig: npt.ArrayLike, u: npt.ArrayLike, v: npt.ArrayLike
    ):
        """
    ...
    # =================================================================
    def distanceFromEllipse(
        p: npt.ArrayLike, p0: npt.ArrayLike, p1: npt.ArrayLike, p2: npt.ArrayLike
    ):
        """
    ...
    def insideParallelepiped(
        ...
        return np.logical_and(
            np.logical_and(uc >= 0, uc <= nu),
            np.logical_and(
                np.logical_and(vc >= 0, vc <= nv), np.logical_and(wc >= 0, wc <= nw)
            ),
        )

    # =================================================================
    def insideSphere(p: npt.ArrayLike, Orig: npt.ArrayLike, r: float):
        """
    ...
    # =================================================================
    def insideCylinder(
        p: npt.ArrayLike, Orig: npt.ArrayLike, axes: npt.ArrayLike, r: float
    ):
        """
        ...
        # radial coordinate
        rc = np.linalg.norm(
            d - np.dot(ac[:, np.newaxis], np.transpose(a[:, np.newaxis])), axis=1
        )
        return np.logical_and(rc <= r, np.logical_and(ac >= 0, ac <= na))

    # =================================================================
    def insideExtrudedEllipse(
        p: npt.ArrayLike,
        p0: npt.ArrayLike,
        p1: npt.ArrayLike,
        p2: npt.ArrayLike,
        ...
        # point p distance to plane
        wc = np.dot(d, w)
        return np.logical_and(np.logical_and(wc >= 0, wc <= h), eq <= 1.0)
