...
if _ts_dolfinx_exist:
    ...
    class scaleJump:
        ...
        def __init__(self, scale_jump):
            ...
            self._coarse_domain = ufl.Mesh(
                basix.ufl.element(
                    "Lagrange",
                    mesh.topology.cell_type.name,
                    1,
                    shape=(mesh.topology.dim,),
    ...
    class enrichedFunction:
        ...
        _enriched_function: typing.Union[
            _ts.enrichedFunction_float32,
            _ts.enrichedFunction_float64,
            _ts.enrichedFunction_complex32,
            _ts.enrichedFunction_complex64,
        ]
    ...
    class patchManager:
        ...
        def generateProblems(self, Aff: PETSc.Mat, bf: PETSc.Vec):
            """
            create patches system
            """
            self._patch_manager.generateProblems(Aff, bf)

        def solveProblems(self, fine_field: Function):
            """
        ...
        def grabPatchSolution(self, seq: int, fine_field: Function):
            """
    ...
    class coarseManager:
        ...
        def setStdCoarse(
            self, Aff: PETSc.Mat, bf: PETSc.Vec, use_imp_enriched: bool = False
        ):
            """
        ...
        def updateEnrichedOperator(self, pm: patchManager, func: enrichedFunction):
            """
            Compute the enriched part of the scale jump operator based on patches solution and the enrichment function generator
            """
            self._coarse_manager.updateEnrichedOperator(
                pm._patch_manager, func._enriched_function
            )

        def updateEnrichCoarse(self, Aff: PETSc.Mat, bf: PETSc.Vec):
            """
            update only enriched part of coarse system
            """
            self._coarse_manager.updateEnrichCoarse(Aff, bf)

        def solve(self, fine_field: Function):
            """
        ...
        def solveEImp(self, fine_field: Function):
            """
        ...
        def projectStdCoarse(
            self, coarse_enriched_field: Function, fine_field: Function
        ):
            """
    ...
    # =================================================================
    def generateMPC(sj: scaleJump, space: FunctionSpace):
        """
        ...
        mpc.add_constraint_from_mpc_data(
            mpc.V, _ts.generateMPC(sj._scale_jump, space._cpp_object)
        )
        mpc.finalize()
        return mpc

    # =================================================================
    def generateCoarseManager(
        sj: scaleJump,
        fine_field: Function,
        ...
        return coarseManager(
            _ts.generateCoarseManager(
                sj._scale_jump,
                fine_field._cpp_object,
                coarse_field._cpp_object,
                None if mpc is None else mpc._cpp_object,
                _bcs,
            )
        )

    # =================================================================
    def generatePatchManager(sj: scaleJump, fine_field: Function):
        """
        Function to create, from scaleJump object , all patches and a manager of them
        """
        return patchManager(
            _ts.generatePatchManager(sj._scale_jump, fine_field._cpp_object)
        )
    ...
    def generateEnrichedShiftFunction(fine_field: Function):
        ...
        return enrichedFunction(
            _ts.generateEnrichedShiftFunction(fine_field._cpp_object)
        )
