...
if _ts_dolfinx_exist:
    ...
    def linearBasicLoop(
        ...
        if do_mpc and mpc is None:
            raise RuntimeError(
                "scale jump provided expect MPC creation and you provide no mpc !"
            )
        ...
        stage.push()
        cm = core.generateCoarseManager(
            sj, fine_space, enriched_field.function_space, mpc, bc
        )
        stage.pop()
    ...
    def linearBasicLoopVideo(
        ...
        while i < itmx and ra > epsr and rds > eps / 100:
            ...
            if master:
                print(
                    f"At twoscale iteration {i} residual is {ra / nb}(b) {ra / nr}(r) {rds}(ds)"
                )
                plt.clear()
                plt.subplot(0, 1)
                chart = pv.Chart2D()
